...
if __name__ == '__main__':
    ...
    labels = []
    for i in range(5000):
        feature, a1, label, a2 = make_data(x, f_num=2, poly_order=6, rdio=[1, 21],
                                           f_range=[[15, 45],
                                                    [50, 150]])
        features.append(feature)
        labels.append(label)

    np.save('../../../Train_new/train_features.npy', features)
    np.save('../../../Train_new/train_labels.npy', labels)

    # 制造1000个测试数据
    features_test = []
    labels_test = []
    for i in range(1000):
        feature, a1, label, a2 = make_data(x, f_num=2, poly_order=6, rdio=[1, 26],
                                           f_range=[[5, 60],
                                                    [100, 250]]
                                           )
        features.append(feature)
    ...
    print(labels[0])
    np.save('../../../Train_new/test_features.npy', features)
    np.save('../../../Train_new/test_labels.npy', labels)
